...
export default function ClientesPage() {
  ...
  // CARGAR HISTORIAL DE UN CLIENTE (NUEVO)
  async function abrirHistorial(cliente: Cliente) {
    ...
    const { data, error } = await supabase
      .from("citas")
      .select(`id, fecha_inicio, servicio, precio, profesionales (nombre)`)
  ...
  async function guardarNotas() {
    ...
    const { error } = await supabase
      .from("clientes")
      .update({ notas: notasEditables })
      .eq("id", clienteDetalle.id);

    if (!error) {
      mostrarNotificacion("Notas actualizadas", "exito");
      cargarClientes(); // Refrescar lista principal
    }
  }
  ...
  async function guardarCliente() {
    ...
    const { error } = await supabase.from("clientes").insert([{ nombre, telefono, email, notas }]);
    if (error) mostrarNotificacion(error.message, "error");
    else {
      setNombre(""); setTelefono(""); setEmail(""); setNotas("");
      setIsModalOpen(false);
      cargarClientes();
      mostrarNotificacion("Cliente guardado", "exito");
    }
  }
  ...
  const clientesFiltrados = clientes.filter(c => 
    c.nombre.toLowerCase().includes(busqueda.toLowerCase()) || (c.telefono && c.telefono.includes(busqueda))
  );

  // CÁLCULOS DE ESTADÍSTICAS (NUEVO)
  const totalInvertido = historialCitas.reduce((acc, cita) => acc + (cita.precio || 0), 0);
  const ultimaVisita = historialCitas.length > 0 ? historialCitas[0].fecha_inicio : null;
    ...
    <div className="flex min-h-screen bg-slate-50 relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto relative">
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-8 shrink-0">
          <h2 className="text-sm font-medium text-slate-500 uppercase tracking-wider">Directorio de Clientes</h2>
          <button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-md">
        ...
        {toast && (
          <div className={`fixed bottom-8 right-8 z-100 p-4 rounded-xl shadow-xl border flex items-center gap-3 animate-in slide-in-from-bottom-5 ${toast.tipo === "error" ? "bg-red-50 text-red-600 border-red-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"}`}>
            <CheckCircle2 size={20} /> <span className="font-bold text-sm">{toast.mensaje}</span>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
              <div className="p-6 text-center flex flex-col items-center">
                ...
                <h3 className="text-xl font-bold text-slate-800">¿Eliminar cliente?</h3>
                <p className="text-slate-500 text-sm mt-2">Esta acción borrará su ficha de forma permanente.</p>
              </div>
              <div className="flex border-t border-slate-100">
                <button onClick={() => setClienteAEliminar(null)} className="flex-1 py-4 text-sm font-bold text-slate-500 hover:bg-slate-50">Cancelar</button>
                <button onClick={confirmarBorrado} className="flex-1 py-4 text-sm font-bold text-red-600 hover:bg-red-50">Sí, eliminar</button>
              </div>
            </div>
          </div>
        )}

        {/* PANEL DE HISTORIAL LATERAL (NUEVO) */}
              ...
              <header className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Miembro desde {format(parseISO(clienteDetalle.id === 'temp' ? new Date().toISOString() : historialCitas[historialCitas.length-1]?.fecha_inicio || new Date().toISOString()), "MMMM yyyy", { locale: es })}</p>
                  </div>
                </div>
